...
def Create(url, headers, data):
    ...
    if ogapi.status_code == 201:
        module.exit_json(changed=True, response=ogapi.json())
    elif ogapi.status_code == 409:
        module.exit_json(changed=False, response=ogapi.json())
    else:
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            operation=dict(required=False, default='list', type='str', choices=["create", "get", "list", "delete","update","gettimeline","export"]),
            details=dict(required=False, type='dict'),
            schedule_id=dict(required=False, type='str'),
            schedule_name=dict(required=False, type='str'),
    ...
    if module.params['operation'] == "create":
        ...
        data = json.dumps(module.params['details'])
        Create(url, headers, data)

    if module.params['operation'] == "get":
        url = "https://%s/v2/schedules/%s?identifierType=name" % (module.params['host'], module.params['schedule_name'])
        headers_dict = {"Content-Type": "application/json", "Authorization": "GenieKey {}" . format(module.params['apiKey'])}
        headers = json.loads(json.dumps(headers_dict))
        Get(url, headers)
    ...
    if module.params['operation'] == "gettimeline":
        url = "https://%s/v2/schedules/%s/timeline?%s?identifierType=name" % (module.params['host'], module.params['schedule_name'], module.params['query'])
        headers_dict = {"Content-Type": "application/json", "Authorization": "GenieKey {}" . format(module.params['apiKey'])}
        headers = json.loads(json.dumps(headers_dict))
        GetTimeline(url, headers)

    if module.params['operation'] == "export":
        url = "https://%s/v2/schedules/%s.ics?identifierType=name" % (module.params['host'], module.params['schedule_name'])
        headers_dict = {"Content-Type": "application/json", "Authorization": "GenieKey {}" . format(module.params['apiKey'])}
        headers = json.loads(json.dumps(headers_dict))
        Export(url, headers)
