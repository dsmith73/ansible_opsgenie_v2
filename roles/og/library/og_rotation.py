...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            rotation_id=dict(required=False, type='str'),
            query=dict(required=False, type='str')
            )
    )

    if module.params['operation'] == "create":
        url = "https://%s/v2/schedules/%s/rotations?scheduleIdentifierType=name" % (module.params['host'], module.params['schedule_name'])
    ...
    if module.params['operation'] == "get":
        url = "https://%s/v2/schedules/%s/rotations/%s?scheduleIdentifierType=name" % (module.params['host'], module.params['schedule_name'], module.params['rotation_id'])
        headers_dict = {"Content-Type": "application/json", "Authorization": "GenieKey {}" . format(module.params['apiKey'])}
        headers = json.loads(json.dumps(headers_dict))
        Get(url, headers)
